...
function onAbilityChanged()
	for _,v in pairs(wildshape_powers.getWindows()) do
...
function addPower(bFocus)
	local w = wildshape_powers.createWindow();
	if bFocus then
...
function updateHeaders()
	...
	local aGroupShown = {};
	for _,nodePower in ipairs(DB.getChildList(getDatabaseNode(), "shapechange.powers")) do
		local sCategory, sGroup, nLevel = self.getWindowSortByNode(nodePower);
		
		if not aCategoryWindows[sCategory] then
			local wh = wildshape_powers.createWindowWithClass("power_group_header");
			if wh then
